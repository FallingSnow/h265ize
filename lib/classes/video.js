            ...
            function normalize() {
                ...
                // "simple" audio RMS-based normalization to -2.0 dBFS
                    ...
                    if (!availableFilters.volumedetect || !availableFilters.volume) {
                        reject(new Error('Installed ffmpeg version does not support volumedetect audio filter or volume audio filter.'));
                    }
            ...
            _self.ffmpegCommand
                ...
                .on('error', function(err, stdout, stderr) {
                    ...
                    if (err.message.startsWith('ffmpeg was killed with signal'))
                        ...
                        reject({message: 'ffmpeg exited with an error', stack: stderr});
                });
