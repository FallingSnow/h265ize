            ...
            function normalize() {
                ...
                if (_self.options.normalizeLevel >= 5) {
                    ...
                    if (!availableFilters.dynaudnorm) {
                        reject(new Error('Installed ffmpeg version does not support the dynaudnorm audio filter.'));
                    }

                    reject(new Error('dynaudnorm is not currently implemented.'));
                ...
                //
                    ...
                    if (!availableFilters.loudnorm) {
                        reject(new Error('Installed ffmpeg version does not support the loudnorm audio filter.'));
                    }
                ...
                // "simple" audio RMS-based normalization to -2.0 dBFS
                    ...
                    if (!availableFilters.volumedetect || !availableFilters.volume) {
                        reject(new Error('Installed ffmpeg version does not support volumedetect audio filter and/or volume audio filter.'));
                    }
            ...
            _self.ffmpegCommand
                ...
                .on('error', function(err, stdout, stderr) {
                    ...
                    if (err.message.startsWith('ffmpeg was killed with signal'))
                        ...
                        reject({
                            message: 'ffmpeg exited with an error',
                            stack: stderr
                        });
                });
